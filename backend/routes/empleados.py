...
def registrar_empleado():
    ...
    # Validar fotos (aceptamos múltiples imágenes bajo la key 'foto')
    fotos = request.files.getlist('foto')
    if not fotos:
        return jsonify({'error': 'Fotos del empleado requeridas'}), 400

    nombre = request.form['nombre'].strip()

    # Filtrar archivos vacíos
    fotos_validas = [f for f in fotos if f.filename and f.filename.strip() != '']
    if not fotos_validas:
        return jsonify({'error': 'Selecciona al menos una foto válida'}), 400
    ...
    for f in fotos_validas:
        ...
        resultados_caras.append(resultado_cara)

    return jsonify({
        'ok':       True,
        'empleado': resultado_emp['empleado'],
        'caras':    resultados_caras,
        'mensaje':  f'Empleado {nombre} registrado con {len(resultados_caras)} fotos'
    }), 201
